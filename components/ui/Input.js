...
export default class Input extends Component {
  ...
  static defaultProps = {
	...
	render() {
		...
		let errorMessage = this.props.errorMessage || 'required';
        ...
        <div className={`${this.props.icon ? 'iconic-input' : ''}`}>
          {icon}
          <input type={format} autocomplete="off" className={`form-control ${this.props.rounded ? 'rounded' : ''}`} onChange={(e)=>this.handleChange(e)} {...this.props} />
          {error}
        </div>
      </div>
		);
